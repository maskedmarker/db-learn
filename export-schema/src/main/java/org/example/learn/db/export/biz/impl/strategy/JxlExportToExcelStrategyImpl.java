...
public class JxlExportToExcelStrategyImpl implements ExportToExcelStrategy {
    ...
    @Override
    public void doExport(String filePath, List<TableInfo> tableInfos) {
        try {
            // 创建写工作簿对象
            ...
            // 添加文档信息sheet
            doDocInfoSheet(workbook, INFO_SHEET_NO, "文档信息");

            // 添加数据库表列表sheet
            doTableListSheet(workbook, LIST_SHEET_NO, "数据表", tableInfos);

            // 添加数据库表字段sheet
            int lastSheetNo = DETAIL_SHEET_NO_START;
...
//            Label labelTitle = new Label(i, LIST_ROW_NO_TABLE_TITLE, tableHeads[i]);
    ...
    private void doTableDetailSheet(WritableWorkbook workbook, int index, TableInfo tableInfo) throws WriteException {
        ...
        workbook.setColourRGB(jxl.format.Colour.LIGHT_TURQUOISE, color.getRed(), color.getGreen(), color.getBlue());
...
//        sheet.addCell(label);
        ...
        // 根据表名,为数据表添加指向当前sheet的超级链接
        addHyperLinkForSheet(workbook.getSheet(LIST_SHEET_NO), sheet, tableInfo.getTableName());
    }

    private void addHyperLinkForSheet(WritableSheet listSheet, WritableSheet detailSheet, String tableName) throws WriteException {
        for (int i = 0; i < listSheet.getRows(); i++) {
            ...
            if (tableName.equals(tableNameCell.getContents())) {
                // replace label with hyperlink
                WritableHyperlink sheetLink = new WritableHyperlink(tableNameCell.getColumn(), tableNameCell.getRow(), tableName, detailSheet, 0, 0);
                listSheet.addHyperlink(sheetLink);
            }
        }
    }
